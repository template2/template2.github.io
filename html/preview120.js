/* -----------------------------------------
	120 Responsive Femine Tistory Skin
	2022.01.19
...
window.addEventListener('DOMContentLoaded', function(){
	...
	//1단 검색버튼
	if(document.querySelector('#wrap .search') != null){
...
function modeSidebar(){
	if(isMobile || document.querySelector('html.sidebar-hide') != null){
		//사이드바1 헤더로 이동
	...
	//검색폼 위치 이동
	if(isMobile){
		let searchform = document.querySelector('nav.category-menu > .inner > .search');
		if(searchform != null)
			document.querySelector('.content-wrap > .inner').appendChild(searchform);
	}else{
		let searchform = document.querySelector('div.content-wrap > .inner > .search');
		if(searchform != null)
			document.querySelector('nav.category-menu > .inner').appendChild(searchform);
	}
}
