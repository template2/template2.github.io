(function($) {
	...
	function setNoImage(){
		$(".cover-list figure").each(function(){
			if(!$(this).find('img').length){
				$(this).append('<img src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==">');
	...
	//아이템 넘버링
	function itemNumbering(){

		//썸네일 숫자 표시용
		$items = $('.post-item > a .numbering:not(".left"):not(".right"), .cover-list ul li a .numbering:not(".left"):not(".right")');

		$items.each(function(){
			var content = ($itemindex+1) < 10 ? '0'+($itemindex+1):($itemindex+1);
			$(this).text(content);
			if($itemindex%2 == 0){
				//짝수
				$(this).addClass('left');
