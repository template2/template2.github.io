(function($) {
	...
	function setNoImage(){
		$(".post-item, .recent-posts > li, .cover-list ul li").each(function(){
			if(!$(this).find('.thum img').length){
				if(!$('body.no-place-thumbnail').length){
					if($('body.short-excerpt').length){
						$(this).find('.thum').append('<img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAADCAQAAAAe/WZNAAAADklEQVR42mNkgAJGDAYAAFEABCaLYqoAAAAASUVORK5CYII=">');
	...
	function checkMobile(){
		...
		if ( (/MSIE (\d+\.\d+);/.test(navigator.userAgent) || navigator.userAgent.indexOf("Trident/") > -1) && $(window).width() > 750 ){ 
			$is_mobile = false;
		}
	}
