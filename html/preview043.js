(function($) {
	...
	function setDateFormat(){
		...
		$(".post-item .date").each(function(){
			if($(this).html().length>5){
				var yyyymmdd = $(this).html().replace(/. /gi,'.');
				$(this).html('<div>'+yyyymmdd.substring(5,7)+'월<br/><span class="dd">'+yyyymmdd.substring(8,10)+'일</span></div>');
			}
		})	
	}

	function setNoImage(){
		$(".post-item.init .thum").each(function(){
			if(!$(this).find('img').length){
				$(this).prepend('<img class="noimage" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAMAAAACCAQAAAA3fa6RAAAADklEQVR42mNkAANGCAUAACMAA2w/AMgAAAAASUVORK5CYII=">');
			}
		})	
	}
	...
	function setMobileCategory(){
		if($('#header .category-menu').css('display') == 'none'){
			if($('.category-menu > .inner > .category').length){
				$('#wrap_left_category').append($('<div id="mobile_category" class="box_aside"><div class="tit_aside">Category</div></div>'));
				$('.category-menu > .inner > .category').detach().appendTo('#mobile_category');
			}
		}else{
			if($('#wrap_left_category > #mobile_category').length){
	...
	function setGridMasonry(b_more){
		...
		objs.each(function(i,el){
			...
			//보이게 처리 - forefox는 hide 상태에서 높이값 체크 불가
			$(this).show();
