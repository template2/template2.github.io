(function($) {
	...
	function gnb(){
		...
		//전역 클릭 다음 이동
		$(document).on("click", ".article-info.current", function(){
			$( ".article-info.current .next" ).trigger( "click" );
			return false;
		});
	}
	...
	function loadImage(b_more){
		...
		if(!$('#content .inner div').length){
			...
			$('#content .inner').prepend('<div class="post-item" style="background-image:url('+url+'error.jpg);"><div class="article-info"><div class="inner"><span class="title">표시할 내용이 없습니다.</span></div></div></div>');
