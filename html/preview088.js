(function($) {
	...
	// 088 Responsive Inline

	var $is_mobile = false;//모바일 기기 체크
	checkMobile()

	var bSidebarCategory = $('body.category-top').length ? false:true;//카테고리 방향 결정
	...
	function setMobileCategory(){
		if($('#header .category-menu').css('display') == 'none' || bSidebarCategory || $is_mobile){
			if($('.category-menu > .inner > .category').length){
				if($('body.category-side1').length){
					$('#profile').before($('<div id="mobile_category" class="box_aside"><div class="tit_aside">Category</div></div>'));
				}else{
					if($is_mobile){
						$('#profile').after($('<div id="mobile_category" class="box_aside"><div class="tit_aside">Category</div></div>'));
					}else{
						$('#search').after($('<div id="mobile_category" class="box_aside"><div class="tit_aside">Category</div></div>'));
					}
				}
	...
	function setSidebar(){
		if($is_mobile){
			...
			//사이드바1,2 상하 위치 구분해 처리
			if($('body.sidebar-swap').length){
				if($('body.sidebar1-bottom').length){
					$('#sidebar_left').detach().insertAfter('.container');
				}
				if($('body.sidebar2-bottom').length){
					$('#sidebar_right').detach().insertAfter('.container');
				}else{
					$('#sidebar_right').detach().prependTo('.container > .inner');
				}
			}else{
				if($('body.sidebar2-bottom').length){
					$('#sidebar_right').detach().insertAfter('.container');
