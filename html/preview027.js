(function($) {
	...
	//썸네일 리스트 위치 변경
	$('.cover-thumbnail-list').detach().prependTo('#footer > .inner');
	...
	function setMobileCategory(){
		...
		if($is_mobile){
			if($('.mobile-menu').css('display') == 'block'){
				$('.category-menu > .inner > .category').detach().prependTo('#gnb');
			}
		}else{
			if($('.category-menu').css('display') != 'block'){
				$('#gnb > .category').detach().prependTo('.category-menu > .inner');
			}
		}
	}

	function setBannerImage(){
		var fi = $('.entry-content').find('img');
		if(fi.length){
			$('.pagination').prepend('<div id="img-banner" class="img-banner"><div></div></div>');
			$('#img-banner').css('background-image','url('+$(fi).attr('src')+')');
		}
	}
